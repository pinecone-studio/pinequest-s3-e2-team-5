...
export default function StudentPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const fullName =
      `${resolvedFormValues.lastName.trim()} ${resolvedFormValues.firstName.trim()}`.trim();
    ...
    if (signIn.status === "complete") {
      await signIn.finalize({
        navigate: ({ session, decorateUrl }) => {
          ...
          const targetUrl =
            rawRole === "student" ? "/student/account" : "/dashboard";
        ...
        <div className="order-1 w-full max-w-[420px] justify-self-end lg:order-2">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2 rounded-[20px] bg-[#F4F0FF] p-1.5">
            <button
              ...
              className={`h-11 rounded-[16px] text-[15px] font-semibold transition ${
                mode === "sign-up"
